...
export function OperationalExpensesWorkspace() {
  ...
  const draftPct = totalRows > 0 ? Math.round((draftCount / totalRows) * 100) : 0;
  const postedPct = totalRows > 0 ? Math.round((postedCount / totalRows) * 100) : 0;
  const voidPct = totalRows > 0 ? Math.round((voidCount / totalRows) * 100) : 0;
    ...
    >
      <div className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricCard
            title="Total Opex"
            value={`${totalRows} transaksi`}
            subtitle={formatRupiah(totalAmount)}
          ...
          <MetricCard
            ...
            subtitle={totalRows > 0 ? `${draftPct}% dari total` : "Belum ada transaksi"}
          />
          <MetricCard
            title="Posted"
            value={String(postedCount)}
            subtitle={totalRows > 0 ? `${postedPct}% dari total` : "Belum ada transaksi"}
          />
          <MetricCard
            title="Void"
            value={String(voidCount)}
            subtitle={totalRows > 0 ? `${voidPct}% dari total` : "Belum ada transaksi"}
          />
        </div>
