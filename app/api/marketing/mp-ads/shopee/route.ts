...
export async function GET() {
  try {
    ...
    const items = await prisma.mp_ads_shopee.findMany({
      orderBy: [{ date: "desc" }, { produk: "asc" }],
    });

    return NextResponse.json(toJsonValue(items));
  } catch (error) {
    return jsonError(error, "Gagal memuat data iklan Shopee.");
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.mp_ads_shopee.create({
      data: {
    ...
    return jsonError(error, "Gagal menyimpan data iklan Shopee.");
  }
}