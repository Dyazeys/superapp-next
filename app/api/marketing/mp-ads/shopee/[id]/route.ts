...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const item = await prisma.mp_ads_shopee.update({
      where: { id },
    ...
    return jsonError(error, "Gagal memperbarui data iklan Shopee.");
  }
}
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    await prisma.mp_ads_shopee.delete({ where: { id } });

    return NextResponse.json({ success: true });
  } catch (error) {
    return jsonError(error, "Gagal menghapus data iklan Shopee.");
  }
}